...
## 5. 数据库恢复 (核心资产)
如果你下载的包里没有 `platform.db`（因为它是二进制文件，有时会被系统隐藏），请直接使用项目根目录下的 **`init_database.sql`**。

**恢复步骤：**
1. 在你的服务器上安装 SQLite (或使用在线 SQLite 工具)。
2. 打开 `init_database.sql`，复制全部内容。
3. 执行该 SQL 脚本，它将自动创建所有表结构，并插入加密后的初始账号。
4. 这样生成的数据库与我为你开发的一模一样，且符合最新的安全标准。
